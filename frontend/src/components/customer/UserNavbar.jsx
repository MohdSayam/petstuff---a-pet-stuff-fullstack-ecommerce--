...
import { Search, ShoppingCart, User, Menu, X, Flame, Sparkles, Home, ShoppingBag } from 'lucide-react';
        ...
        <nav className='bg-white border-b border-slate-100 w-full sticky top-0 z-50'>
            ...
            <div className='w-full px-6 h-20 flex items-center justify-between gap-8'>
                ...
                <div className='flex items-center gap-4'>
                    ...
                    <Link to="/cart" className="relative p-3 bg-slate-900 text-white rounded-2xl hover:bg-brand-primary transition-all shadow-lg shadow-slate-900/20">
                ...
                <div className={`absolute top-0 left-0 bottom-0 w-80 bg-white shadow-2xl transition-transform duration-300 flex flex-col ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    ...
                    <div className='flex-1 overflow-y-auto p-6'>
                        ...
                        <nav className='space-y-8'>
                            <div>
                                <h3 className='text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4'>Menu</h3>
                                <div className='grid gap-4'>
                                    <Link onClick={toggleMenu} to="/" className='flex items-center gap-3 font-bold text-slate-700'><Home size={18}/> Home</Link>
                                    <Link onClick={toggleMenu} to="/shop" className='flex items-center gap-3 font-bold text-slate-700'><ShoppingBag size={18}/> Shop All</Link>
                                </div>
                            </div>
                        </nav>
                    </div>
