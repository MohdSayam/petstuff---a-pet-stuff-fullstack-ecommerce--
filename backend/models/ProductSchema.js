...
const productSchema = new mongoose.Schema({
  ...
  salePrice: { type: Number }, // Calculated automatically
...
productSchema.pre("save", function (next) {
  ...
  if (!this.discountPercentage || this.discountPercentage === 0) {
    ...
    // Calculate salePrice based on percentage
    // Formula: Sale Price = Original - (Original * (Discount / 100))
    const discountAmount = (this.originalPrice * this.discountPercentage) / 100;
    this.salePrice = Math.round((this.originalPrice - discountAmount) * 100) / 100;
  }
  next();
});
